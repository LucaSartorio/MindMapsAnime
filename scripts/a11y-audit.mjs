...
const scenarios = [
  ...
  { name: 'filters drawer', go: async (p) => { await p.goto(BASE + '/worlds/naruto'); await p.waitForTimeout(700); await accept(p, /accetta tutti|accept all/i); await accept(p, /start exploring|inizia|esplora/i); await p.getByRole('button', { name: /^filtri$|^filters$/i }).first().click().catch(()=>{}); await p.waitForTimeout(400); const chip = p.locator('aside[role="dialog"] button[aria-pressed="false"]'); if (await chip.count()) await chip.first().click().catch(()=>{}); await p.waitForTimeout(300); } },
  { name: 'layers drawer', go: async (p) => { await p.goto(BASE + '/worlds/naruto'); await p.waitForTimeout(700); await accept(p, /accetta tutti|accept all/i); await accept(p, /start exploring|inizia|esplora/i); await p.getByRole('button', { name: /^livelli$|^layers$/i }).first().click().catch(()=>{}); await p.waitForTimeout(400); } },
  { name: 'map panels (legend/routes/timeline)', go: async (p) => { await p.goto(BASE + '/worlds/naruto'); await p.waitForTimeout(700); await accept(p, /accetta tutti|accept all/i); await accept(p, /start exploring|inizia|esplora/i); await p.getByRole('button', { name: /legenda|legend/i }).first().click().catch(()=>{}); await p.getByRole('button', { name: /percorsi|routes/i }).first().click().catch(()=>{}); await p.getByRole('button', { name: /timeline/i }).first().click().catch(()=>{}); await p.waitForTimeout(500); } },
  { name: 'location modal (tabs)', go: async (p) => { await p.goto(BASE + '/worlds/naruto'); await p.waitForTimeout(800); await accept(p, /accetta tutti|accept all/i); await accept(p, /start exploring|inizia|esplora/i); await p.locator('.react-flow__node-map-node .pointer-events-auto').first().click().catch(()=>{}); await p.waitForTimeout(600); } },
  { name: 'route modal (stepper)', go: async (p) => { await p.goto(BASE + '/worlds/naruto'); await p.waitForTimeout(800); await accept(p, /accetta tutti|accept all/i); await accept(p, /start exploring|inizia|esplora/i); await p.getByRole('button', { name: /^percorsi$|^routes$/i }).first().click().catch(()=>{}); await p.waitForTimeout(300); await p.getByRole('button', { name: /dettagli percorso|route details/i }).first().click().catch(()=>{}); await p.waitForTimeout(600); } },
  { name: 'story mode', go: async (p) => { await p.goto(BASE + '/worlds/naruto/arcs'); await accept(p, /accetta tutti|accept all/i); await p.waitForTimeout(500); await p.locator('main button').filter({ hasText: /.{2,}/ }).first().click().catch(()=>{}); await p.waitForTimeout(500); await p.getByRole('button', { name: /avvia storia guidata|start guided story/i }).first().click().catch(()=>{}); await p.waitForTimeout(800); } },
  { name: 'relations graph', go: async (p) => { await p.goto(BASE + '/worlds/naruto/characters'); await accept(p, /accetta tutti|accept all/i); await p.waitForTimeout(500); await p.locator('main button').filter({ hasText: /.{2,}/ }).first().click().catch(()=>{}); await p.waitForTimeout(500); await p.getByRole('button', { name: /mappa delle relazioni|relationship map/i }).first().click().catch(()=>{}); await p.waitForTimeout(700); } },
