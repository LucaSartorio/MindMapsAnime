...
const scenarios = [
  ...
  { name: 'location modal (tabs)', go: async (p) => { await p.goto(BASE + '/worlds/naruto'); await p.waitForTimeout(800); await accept(p, /accetta tutti|accept all/i); await accept(p, /start exploring|inizia|esplora/i); await p.locator('.react-flow__node-map-node').first().click().catch(()=>{}); await p.waitForTimeout(600); } },
