import type { Location } from '@/types';

/**
 * Luoghi di Paradise — la prima metà della Grand Line, da Reverse Mountain alla
 * Red Line. È la fascia orizzontale a destra del centro mappa; le isole sono
 * fitte e ravvicinate, quindi le coordinate qui vanno rifinite con cura sulla
 * mappa ad alta risoluzione. Coordinate nel piano viewBox 2000 × 1000.
 */
export const onepieceLocationsParadise: Location[] = [
  {
    id: 'loc-op-reverse-mountain',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Reverse Mountain',
    localizedName: { it: 'Reverse Mountain', en: 'Reverse Mountain' },
    type: 'mountain',
    x: 1018,
    y: 505,
    shortDescription: {
      it: "L'unico ingresso alla Grand Line dai quattro Mari: una montagna sulla Red Line le cui correnti risalgono i quattro versanti e ridiscendono nella Grand Line.",
      en: 'The only entrance to the Grand Line from the four Seas: a mountain on the Red Line whose currents climb all four slopes and pour down into the Grand Line.',
    },
    nationId: 'nation-op-red-line',
    characterIds: ['char-op-luffy', 'char-op-zoro', 'char-op-nami', 'char-op-usopp', 'char-op-sanji'],
    arcIds: ['arc-op-loguetown'],
    eventIds: ['evt-op-enter-grand-line'],
    importance: 'main',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['grand-line', 'red-line', 'ingresso'],
  },
  {
    id: 'loc-op-twin-cape',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Twin Cape',
    localizedName: { it: 'Capo Gemello', en: 'Twin Cape' },
    type: 'landmark',
    x: 1057,
    y: 514,
    shortDescription: {
      it: "Il faro all'imbocco della Grand Line, casa del guardiano Crocus e della balena Laboon, che da 50 anni attende il ritorno dei suoi pirati.",
      en: 'The lighthouse at the mouth of the Grand Line, home of the keeper Crocus and the whale Laboon, who has awaited her pirates for 50 years.',
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'laboon', 'crocus'],
  },
  {
    id: 'loc-op-whisky-peak',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Whisky Peak (Cactus Island)',
    localizedName: { it: 'Whisky Peak (Isola Cactus)', en: 'Whisky Peak (Cactus Island)' },
    type: 'city',
    x: 1093,
    y: 494,
    shortDescription: {
      it: "Città dei cactus che accoglie i pirati come eroi salvo poi derubarli: covo dei cacciatori di taglie della Baroque Works. Qui Vivi rivela la sua identità.",
      en: 'A cactus-shaped town that welcomes pirates as heroes only to rob them: a nest of Baroque Works bounty hunters. Here Vivi reveals her identity.',
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'baroque-works', 'vivi'],
  },
  {
    id: 'loc-op-little-garden',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Little Garden',
    localizedName: { it: 'Little Garden', en: 'Little Garden' },
    type: 'landmark',
    x: 1120,
    y: 473,
    shortDescription: {
      it: "Isola preistorica fuori dal tempo, popolata da dinosauri e dai due giganti di Elbaf Dorry e Brogy, eterni rivali da cent'anni.",
      en: 'A prehistoric island frozen in time, populated by dinosaurs and by the two Elbaf giants Dorry and Brogy, eternal rivals for a hundred years.',
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'giganti', 'elbaf'],
  },
  {
    id: 'loc-op-drum-island',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    subMapLevelId: 'op-map-drum-island',
    name: 'Drum Island (Sakura Kingdom)',
    localizedName: { it: 'Drum Island (Regno di Sakura)', en: 'Drum Island (Sakura Kingdom)' },
    type: 'city',
    x: 1186,
    y: 461,
    shortDescription: {
      it: "Isola invernale dalle vette innevate, regno dei medici. Qui Rufy recluta Tony Tony Chopper, allievo della dottoressa Kureha e del compianto Hiluluk.",
      en: 'A snow-capped winter island, a kingdom of doctors. Here Luffy recruits Tony Tony Chopper, pupil of Dr. Kureha and the late Hiluluk.',
    },
    nationId: 'nation-op-grand-line-paradise',
    characterIds: ['char-op-chopper', 'char-op-kureha', 'char-op-hiluluk', 'char-op-wapol', 'char-op-dalton', 'char-op-luffy'],
    arcIds: ['arc-op-drum'],
    eventIds: ['evt-op-hiluluk-dream', 'evt-op-chopper-joins'],
    importance: 'main',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'chopper', 'inverno'],
  },
  {
    id: 'loc-op-alabasta',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Sandy Island (Alabasta)',
    localizedName: { it: 'Isola Sandy (Alabasta)', en: 'Sandy Island (Alabasta)' },
    type: 'nation',
    x: 1259,
    y: 477,
    shortDescription: {
      it: "Vasto regno desertico governato dalla famiglia Nefertari. Teatro della guerra civile orchestrata da Crocodile e della Baroque Works, sventata dalla ciurma a fianco della principessa Vivi.",
      en: "A vast desert kingdom ruled by the Nefertari family. Stage of the civil war engineered by Crocodile and Baroque Works, foiled by the crew alongside Princess Vivi.",
    },
    nationId: 'nation-op-grand-line-paradise',
    subMapLevelId: 'op-map-alabasta',
    characterIds: ['char-op-vivi', 'char-op-crocodile', 'char-op-robin', 'char-op-cobra', 'char-op-luffy'],
    arcIds: ['arc-op-alabasta'],
    eventIds: ['evt-op-vivi-farewell'],
    importance: 'main',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'alabasta', 'vivi', 'crocodile', 'deserto'],
  },
  {
    id: 'loc-op-nanohana',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-alabasta',
    name: 'Nanohana',
    localizedName: { it: 'Nanohana', en: 'Nanohana' },
    type: 'city',
    x: 840,
    y: 300,
    shortDescription: {
      it: "Città portuale di Alabasta, prospera per il commercio dei profumi. Qui Rufy si imbatte di nuovo in Ace.",
      en: 'A port town of Alabasta, prosperous from the perfume trade. Here Luffy crosses paths again with Ace.',
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'alabasta'],
  },
  {
    id: 'loc-op-rainbase',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-alabasta',
    name: 'Rainbase',
    localizedName: { it: 'Rainbase', en: 'Rainbase' },
    type: 'city',
    x: 380,
    y: 310,
    shortDescription: {
      it: "Città del gioco d'azzardo di Alabasta, dove sorge il Rain Dinners, il casinò-quartier generale di Crocodile.",
      en: "Alabasta's gambling city, home to Rain Dinners, the casino-headquarters of Crocodile.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'alabasta', 'crocodile'],
  },
  {
    id: 'loc-op-erumalu',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-alabasta',
    name: 'Erumalu',
    localizedName: { it: 'Erumalu', en: 'Erumalu' },
    type: 'ruins',
    x: 300,
    y: 470,
    shortDescription: {
      it: "«La città del verde», un tempo florida e ora ridotta a rovine dalla siccità causata da Crocodile in Alabasta.",
      en: "'The city of green', once flourishing and now reduced to ruins by the drought Crocodile inflicted on Alabasta.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'alabasta', 'rovine'],
  },
  {
    id: 'loc-op-yuba',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-alabasta',
    name: 'Yuba',
    localizedName: { it: 'Yuba', en: 'Yuba' },
    type: 'village',
    x: 760,
    y: 520,
    shortDescription: {
      it: "Oasi di Alabasta dove si raduna l'esercito ribelle, ormai sepolta dalle tempeste di sabbia. Qui resiste Toto.",
      en: "An Alabasta oasis where the rebel army gathers, now buried by sandstorms. Toto holds out here.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'alabasta', 'ribelli'],
  },
  {
    id: 'loc-op-katorea',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-alabasta',
    name: 'Katorea',
    localizedName: { it: 'Katorea', en: 'Katorea' },
    type: 'village',
    x: 840,
    y: 500,
    shortDescription: {
      it: "Città di Alabasta divenuta punto di raccolta dell'esercito ribelle guidato da Koza.",
      en: "An Alabasta town that became the rallying point of the rebel army led by Koza.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'alabasta', 'ribelli'],
  },
  {
    id: 'loc-op-long-ring-long-land',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Long Ring Long Land',
    localizedName: { it: 'Long Ring Long Land', en: 'Long Ring Long Land' },
    type: 'landmark',
    x: 1429,
    y: 498,
    shortDescription: {
      it: "Isola lunghissima e stretta dove si svolge il Davy Back Fight contro la ciurma di Foxy il Volpe Argentata.",
      en: 'A very long, narrow island where the Davy Back Fight against Foxy the Silver Fox is held.',
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'foxy', 'davy-back-fight'],
  },
  {
    id: 'loc-op-jaya',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Jaya',
    localizedName: { it: 'Jaya', en: 'Jaya' },
    type: 'landmark',
    x: 1338,
    y: 525,
    shortDescription: {
      it: "Isola dei pirati e dei cacciatori di sogni: metà di essa fu spazzata in cielo dalla Knock-Up Stream, diventando l'Upper Yard di Skypiea.",
      en: "A pirates' and dreamers' island: half of it was blasted skyward by the Knock-Up Stream, becoming Skypiea's Upper Yard.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'jaya', 'skypiea'],
  },
  {
    id: 'loc-op-mock-town',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Mock Town',
    localizedName: { it: 'Mock Town', en: 'Mock Town' },
    type: 'city',
    x: 1320,
    y: 522,
    shortDescription: {
      it: "Città-covo di pirati su Jaya, dove la ciurma incontra Bellamy e dove Blackbeard pronuncia la sua filosofia dei sogni.",
      en: "A pirate den-town on Jaya, where the crew meets Bellamy and where Blackbeard voices his philosophy of dreams.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'jaya', 'bellamy', 'blackbeard'],
  },
  {
    id: 'loc-op-skypiea',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Skypiea',
    localizedName: { it: 'Skypiea', en: 'Skypiea' },
    type: 'sacred_place',
    x: 1389,
    y: 498,
    shortDescription: {
      it: "Isola del cielo a 10.000 metri d'altitudine, fatta di nuvole. Regno di «Dio» Enel, custode dell'oro della città perduta di Shandora.",
      en: "A sky island at 10,000 metres, made of clouds. The realm of 'God' Enel, keeper of the gold of the lost city of Shandora.",
    },
    nationId: 'nation-op-grand-line-paradise',
    subMapLevelId: 'op-map-skypiea',
    characterIds: ['char-op-enel', 'char-op-gan-fall', 'char-op-wiper', 'char-op-conis', 'char-op-luffy'],
    arcIds: ['arc-op-skypiea'],
    eventIds: ['evt-op-skypiea-enel'],
    importance: 'main',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'cielo', 'enel', 'shandora'],
  },
  {
    id: 'loc-op-weatheria',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Weatheria',
    localizedName: { it: 'Weatheria', en: 'Weatheria' },
    type: 'sacred_place',
    x: 1490,
    y: 467,
    shortDescription: {
      it: "Isola del cielo abitata da scienziati del meteo, dove Nami studia la scienza del clima durante i due anni di separazione.",
      en: "A sky island inhabited by weather scientists, where Nami studies the science of climate during the two-year separation.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'cielo', 'nami'],
  },
  {
    id: 'loc-op-water-seven',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    subMapLevelId: 'op-map-water-seven',
    name: 'Water Seven',
    localizedName: { it: 'Water Seven', en: 'Water Seven' },
    type: 'city',
    x: 1626,
    y: 472,
    shortDescription: {
      it: "«La città dell'acqua»: metropoli di canali e cantieri navali, patria dei migliori carpentieri. Qui nasce la Thousand Sunny e Franky si unisce alla ciurma.",
      en: "'The city of water': a metropolis of canals and shipyards, home of the finest shipwrights. Here the Thousand Sunny is born and Franky joins the crew.",
    },
    nationId: 'nation-op-grand-line-paradise',
    characterIds: ['char-op-franky', 'char-op-iceburg', 'char-op-lucci', 'char-op-kaku', 'char-op-robin', 'char-op-luffy'],
    arcIds: ['arc-op-water-seven', 'arc-op-enies-lobby'],
    eventIds: ['evt-op-robin-taken', 'evt-op-franky-joins'],
    importance: 'main',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'franky', 'thousand-sunny', 'galley-la'],
  },
  {
    id: 'loc-op-san-faldo',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'San Faldo',
    localizedName: { it: 'San Faldo', en: 'San Faldo' },
    type: 'city',
    x: 1554,
    y: 525,
    shortDescription: {
      it: "Città collegata a Water Seven dal Sea Train, nota per i suoi acquedotti e le sue feste.",
      en: 'A city linked to Water Seven by the Sea Train, known for its aqueducts and festivals.',
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'sea-train'],
  },
  {
    id: 'loc-op-st-poplar',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'St. Poplar',
    localizedName: { it: 'St. Poplar', en: 'St. Poplar' },
    type: 'city',
    x: 1608,
    y: 535,
    shortDescription: {
      it: "«La città primaverile», collegata dal Sea Train, salvata dalla ciurma di Franky in passato.",
      en: "'The spring city', linked by the Sea Train, once saved by the Franky Family.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'minor',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'sea-train'],
  },
  {
    id: 'loc-op-enies-lobby',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    subMapLevelId: 'op-map-enies-lobby',
    name: 'Enies Lobby',
    localizedName: { it: 'Enies Lobby', en: 'Enies Lobby' },
    type: 'landmark',
    x: 1728,
    y: 524,
    shortDescription: {
      it: "L'isola giudiziaria del Governo Mondiale, dove «non esiste la notte». Qui la ciurma dichiara guerra al Governo per salvare Nico Robin e brucia la bandiera.",
      en: "The judicial island of the World Government, where 'night never falls'. Here the crew declares war on the Government to save Nico Robin and burns its flag.",
    },
    nationId: 'nation-op-grand-line-paradise',
    characterIds: ['char-op-robin', 'char-op-lucci', 'char-op-spandam', 'char-op-luffy', 'char-op-franky'],
    arcIds: ['arc-op-enies-lobby'],
    eventIds: ['evt-op-declaration-war', 'evt-op-robin-rescue'],
    importance: 'main',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'robin', 'cp9', 'governo-mondiale'],
  },
  {
    id: 'loc-op-thriller-bark',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    subMapLevelId: 'op-map-thriller-bark',
    name: 'Thriller Bark',
    localizedName: { it: 'Thriller Bark', en: 'Thriller Bark' },
    type: 'landmark',
    x: 1794,
    y: 491,
    shortDescription: {
      it: "La più grande nave del mondo, un'isola-galeone avvolta nella nebbia del Triangolo Florian. Regno del Corsaro Gekko Moria, ladro di ombre, dove la ciurma trova Brook.",
      en: "The largest ship in the world, a galleon-island shrouded in the Florian Triangle's fog. Realm of the Warlord Gecko Moria, a stealer of shadows, where the crew finds Brook.",
    },
    nationId: 'nation-op-grand-line-paradise',
    characterIds: ['char-op-brook', 'char-op-moria', 'char-op-perona', 'char-op-luffy', 'char-op-zoro', 'char-op-kuma'],
    arcIds: ['arc-op-thriller-bark'],
    eventIds: ['evt-op-moria-defeat', 'evt-op-brook-joins'],
    importance: 'main',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'moria', 'brook', 'florian'],
  },
  {
    id: 'loc-op-florian-triangle',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Florian Triangle',
    localizedName: { it: 'Triangolo Florian', en: 'Florian Triangle' },
    type: 'region',
    x: 1768,
    y: 448,
    shortDescription: {
      it: "Tratto di mare perennemente avvolto da una fitta nebbia tra Water Seven e Fish-Man Island, dove spariscono navi ed equipaggi.",
      en: 'A stretch of sea perpetually shrouded in dense fog between Water Seven and Fish-Man Island, where ships and crews vanish.',
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'nebbia', 'florian'],
  },
  {
    id: 'loc-op-lulusia-kingdom',
    worldId: 'world-onepiece',
    mapLevelId: 'op-map-world',
    name: 'Lulusia Kingdom',
    localizedName: { it: 'Regno di Lulusia', en: 'Lulusia Kingdom' },
    type: 'city',
    x: 1855,
    y: 476,
    shortDescription: {
      it: "Piccolo regno di Paradise vicino alla Calm Belt, cancellato dall'esistenza dal potere segreto di Imu attraverso un'arma ancestrale.",
      en: "A small Paradise kingdom near the Calm Belt, wiped from existence by Imu's secret power through an ancient weapon.",
    },
    nationId: 'nation-op-grand-line-paradise',
    importance: 'secondary',
    canonStatus: 'canon',
    referenceStatus: 'verified',
    tags: ['paradise', 'imu', 'governo-mondiale'],
  },
];
